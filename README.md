...
4. **进行面试**：浏览器麦克风实时上行 → ASR 流式识别 → AI 面试官追问 → TTS 流式播报
5. **生成报告**：结束后出总结 / 亮点 / 不足 / 建议 + 逐轮点评 + 分数曲线
6. **历史管理**：所有面试记录可查可删

**定位**：个人练习 / 内部演示用，不面向多租户生产负载（SQLite + 单机）。

**在线访问**：已部署至 [https://www.qiinterview.com](https://www.qiinterview.com)（单机 ECS + SQLite，仅供个人体验；如遇不可用以本地启动为准，见 §6）。

---

## 2. 界面预览

以下截图来自实际运行界面（代码对应页面见 §4.9）：

| 页面 | 截图 |
|---|---|
| 登录（`/login`） | ![登录页](screenshots/QiInterview01.png) |
| 预约 / 准备面试（`/setup`） | ![预约页](screenshots/QiInterview02.png) |
| 实时面试（`/interview/:sid`） | ![面试页](screenshots/QiInterview03.png) |
| 复盘报告 · 总览（`/report/:sid`） | ![复盘页1](screenshots/QiInterview04.png) |
| 复盘报告 · 逐轮点评（`/report/:sid`） | ![复盘页2](screenshots/QiInterview05.png) |
...
### 4.1 账号与鉴权（[app/api/auth.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/api/auth.py)）
...
### 4.2 用户凭据（[app/api/credentials.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/api/credentials.py)）

- 业务代码运行时从 `UserCredential` 表按 `current_user` 读取，不回退读 `.env`（`.env` 里的 Key 仅 E2E 脚本使用）

### 4.3 岗位库（[app/services/jobs/](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/services/jobs)，[app/api/jobs.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/api/jobs.py)）
...
### 4.4 面试会话（[app/api/interview.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/api/interview.py)）
...
### 4.5 简历上传（[app/api/resume.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/api/resume.py)）

- `POST /resume/upload`：上传 PDF → 后端用 `pypdf` 抽文本 → LLM 结构化摘要供后续追问引用
- 简历文本按会话持久化（`InterviewSession.resume_text`）

### 4.6 面试过程（[app/api/voice_ws.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/api/voice_ws.py) + [app/services/interviewer.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/services/interviewer.py)）
...
### 4.7 报告（[app/api/reports.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/api/reports.py) + [app/services/report.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/services/report.py)）
...
### 4.8 传输与部署安全（[app/main.py](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/backend/app/main.py)）
...
### 4.9 前端页面（[frontend/src/pages/](file:///e:/works/codes/Python/Qoder/try2/QiInterview2/frontend/src/pages)）
